...
extension Reactive where Base: UICollectionView {
    ...
    public func modelSelected<T>(_ modelType: T.Type) -> ControlEvent<T> {
        let source: Observable<T> = itemSelected.flatMap { [weak championView = self.base as UICollectionView] indexPath -> Observable<T> in
            guard let championView = championView else {
                return Observable.empty()
            }

            return Observable.just(try championView.rx.model(at: indexPath))
        }
    ...
    public func modelDeselected<T>(_ modelType: T.Type) -> ControlEvent<T> {
        let source: Observable<T> = itemDeselected.flatMap { [weak championView = self.base as UICollectionView] indexPath -> Observable<T> in
            guard let championView = championView else {
                return Observable.empty()
            }
